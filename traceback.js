/**
 * Traceback Manager for Knapsack and LCS algorithms
 * Handles the traceback visualization with logging and monk mode support
 */
class TracebackManager {
    constructor(visualizer) {
        this.visualizer = visualizer;
        this.dpTable = null;
        this.algoType = null;
        this.algoData = null;
        this.isTracing = false;
        this.isMonkMode = false;
        this.tracebackSteps = [];
        this.currentTraceStep = 0;
        this.currentRow = 0;
        this.currentCol = 0;
        this.selectedItems = [];
        this.lcsResult = '';
        this.animationSpeed = 500;
        this.expectedNextCell = null;
        this.cellClickHandler = null;

        // DOM Elements
        this.logContainer = document.getElementById('traceback-log');
        this.tracebackCard = document.getElementById('traceback-card');
        this.tracebackResult = document.getElementById('traceback-result');
        this.selectedItemsEl = document.getElementById('selected-items');
        this.tracebackBtn = document.getElementById('traceback-btn');
        this.closeBtn = document.getElementById('close-traceback-btn');

        this._initEventListeners();
    }

    _initEventListeners() {
        if (this.closeBtn) {
            this.closeBtn.addEventListener('click', () => this.stopTraceback());
        }
    }

    /**
     * Initialize traceback with solved DP table
     */
    init(algoType, dpTable, algoData, isMonkMode = false) {
        this.algoType = algoType;
        this.dpTable = dpTable;
        this.algoData = algoData;
        this.isMonkMode = isMonkMode;
        this.selectedItems = [];
        this.lcsResult = '';
        this.tracebackSteps = [];
        this.currentTraceStep = 0;

        // Clear previous state
        this.logContainer.innerHTML = '';
        this.tracebackResult.classList.add('hidden');
        this.selectedItemsEl.textContent = '';
    }

    /**
     * Start the traceback process
     */
    startTraceback() {
        this.isTracing = true;
        this.tracebackCard.classList.remove('hidden');
        this.tracebackBtn.classList.add('active');
        this.tracebackBtn.textContent = '⏳ Tracing...';

        // Clear any previous highlights
        this._clearTracebackHighlights();

        if (this.algoType === 'knapsack') {
            this._startKnapsackTraceback();
        } else {
            this._startLCSTraceback();
        }
    }

    /**
     * Knapsack Traceback Logic
     */
    _startKnapsackTraceback() {
        const { weights, values, capacity } = this.algoData;
        const n = weights.length;

        this.currentRow = n;
        this.currentCol = capacity;

        // Add initial log entry
        this._addLogEntry({
            step: 'Start',
            position: `DP[${n}][${capacity}]`,
            condition: `Starting at bottom-right cell with value ${this.dpTable[n][capacity]}`,
            path: 'Beginning traceback...',
            type: 'start'
        });

        // Highlight starting cell
        this._highlightCurrentCell(n, capacity);

        if (this.isMonkMode) {
            this._addMonkHint('Click the cell that traceback should visit next');
            this._setupMonkModeKnapsack();
        } else {
            this._animateKnapsackTraceback(n, capacity);
        }
    }

    /**
     * Animate knapsack traceback automatically
     */
    async _animateKnapsackTraceback(i, w) {
        const { weights, values } = this.algoData;

        while (i > 0 && w > 0) {
            await this._delay(this.animationSpeed);

            const itemWeight = weights[i - 1];
            const itemValue = values[i - 1];
            const currentVal = this.dpTable[i][w];
            const aboveVal = this.dpTable[i - 1][w];

            // Mark current as visited
            this._markAsVisited(i, w);

            if (currentVal !== aboveVal) {
                // Item was included
                this.selectedItems.push(i);

                this._addLogEntry({
                    step: `Item ${i}`,
                    position: `DP[${i}][${w}] = ${currentVal}`,
                    condition: `DP[${i}][${w}] (${currentVal}) ≠ DP[${i - 1}][${w}] (${aboveVal})`,
                    path: `✓ Include Item ${i} (Weight: ${itemWeight}, Value: $${itemValue})`,
                    type: 'include'
                });

                w -= itemWeight;
                i -= 1;

                if (i > 0 || w > 0) {
                    this._highlightCurrentCell(i, w);
                }
            } else {
                // Item was not included
                this._addLogEntry({
                    step: `Item ${i}`,
                    position: `DP[${i}][${w}] = ${currentVal}`,
                    condition: `DP[${i}][${w}] (${currentVal}) = DP[${i - 1}][${w}] (${aboveVal})`,
                    path: `✗ Skip Item ${i} → Move to DP[${i - 1}][${w}]`,
                    type: 'exclude'
                });

                i -= 1;

                if (i > 0) {
                    this._highlightCurrentCell(i, w);
                }
            }
        }

        this._completeTraceback();
    }

    /**
     * Setup monk mode for knapsack - user clicks cells
     */
    _setupMonkModeKnapsack() {
        const { weights, values } = this.algoData;
        let i = this.currentRow;
        let w = this.currentCol;

        const processStep = (clickedRow, clickedCol) => {
            if (i <= 0 || w <= 0) {
                this._completeTraceback();
                return;
            }

            const itemWeight = weights[i - 1];
            const itemValue = values[i - 1];
            const currentVal = this.dpTable[i][w];
            const aboveVal = this.dpTable[i - 1][w];

            let expectedRow, expectedCol;
            let isInclude;

            if (currentVal !== aboveVal) {
                // Should include item
                expectedRow = i - 1;
                expectedCol = w - itemWeight;
                isInclude = true;
            } else {
                // Should skip item
                expectedRow = i - 1;
                expectedCol = w;
                isInclude = false;
            }

            if (clickedRow === expectedRow && clickedCol === expectedCol) {
                // Correct click
                this._markAsVisited(i, w);

                if (isInclude) {
                    this.selectedItems.push(i);
                    this._addLogEntry({
                        step: `Item ${i}`,
                        position: `DP[${i}][${w}] = ${currentVal}`,
                        condition: `DP[${i}][${w}] (${currentVal}) ≠ DP[${i - 1}][${w}] (${aboveVal})`,
                        path: `✓ Include Item ${i} (Weight: ${itemWeight}, Value: $${itemValue})`,
                        type: 'include'
                    });
                } else {
                    this._addLogEntry({
                        step: `Item ${i}`,
                        position: `DP[${i}][${w}] = ${currentVal}`,
                        condition: `DP[${i}][${w}] (${currentVal}) = DP[${i - 1}][${w}] (${aboveVal})`,
                        path: `✗ Skip Item ${i} → Move up`,
                        type: 'exclude'
                    });
                }

                i = expectedRow;
                w = expectedCol;

                if (i <= 0 || w <= 0) {
                    this._completeTraceback();
                } else {
                    this._highlightCurrentCell(i, w);
                    this._enableCellClicks(processStep);
                }
            } else {
                // Wrong click
                this._showWrongClick(clickedRow, clickedCol);
            }
        };

        this._enableCellClicks(processStep);
    }

    /**
     * LCS Traceback Logic
     */
    _startLCSTraceback() {
        const { s1, s2 } = this.algoData;
        const m = s1.length;
        const n = s2.length;

        this.currentRow = m;
        this.currentCol = n;
        this.lcsResult = '';

        // Add initial log entry
        this._addLogEntry({
            step: 'Start',
            position: `DP[${m}][${n}]`,
            condition: `Starting at bottom-right cell. LCS length = ${this.dpTable[m][n]}`,
            path: 'Beginning traceback...',
            type: 'start'
        });

        // Highlight starting cell
        this._highlightCurrentCell(m, n);

        if (this.isMonkMode) {
            this._addMonkHint('Click the cell that traceback should visit next');
            this._setupMonkModeLCS();
        } else {
            this._animateLCSTraceback(m, n);
        }
    }

    /**
     * Animate LCS traceback automatically
     */
    async _animateLCSTraceback(i, j) {
        const { s1, s2 } = this.algoData;
        let lcsChars = [];

        while (i > 0 && j > 0) {
            await this._delay(this.animationSpeed);

            const char1 = s1[i - 1];
            const char2 = s2[j - 1];

            // Mark current as visited
            this._markAsVisited(i, j);

            if (char1 === char2) {
                // Characters match - include in LCS
                lcsChars.unshift(char1);

                this._addLogEntry({
                    step: `Match`,
                    position: `DP[${i}][${j}] comparing '${char1}' with '${char2}'`,
                    condition: `X[${i - 1}] = '${char1}' == Y[${j - 1}] = '${char2}'`,
                    path: `✓ Match! Add '${char1}' to LCS → Go diagonal to DP[${i - 1}][${j - 1}]`,
                    type: 'match'
                });

                i -= 1;
                j -= 1;

                if (i > 0 && j > 0) {
                    this._highlightCurrentCell(i, j);
                }
            } else {
                // No match - take max of top or left
                const topVal = this.dpTable[i - 1][j];
                const leftVal = this.dpTable[i][j - 1];

                if (topVal >= leftVal) {
                    this._addLogEntry({
                        step: `No Match`,
                        position: `DP[${i}][${j}] comparing '${char1}' with '${char2}'`,
                        condition: `'${char1}' ≠ '${char2}'. Top=${topVal} ≥ Left=${leftVal}`,
                        path: `→ Move up to DP[${i - 1}][${j}]`,
                        type: 'move-up'
                    });
                    i -= 1;
                } else {
                    this._addLogEntry({
                        step: `No Match`,
                        position: `DP[${i}][${j}] comparing '${char1}' with '${char2}'`,
                        condition: `'${char1}' ≠ '${char2}'. Top=${topVal} < Left=${leftVal}`,
                        path: `→ Move left to DP[${i}][${j - 1}]`,
                        type: 'move-left'
                    });
                    j -= 1;
                }

                if (i > 0 && j > 0) {
                    this._highlightCurrentCell(i, j);
                }
            }
        }

        this.lcsResult = lcsChars.join('');
        this._completeTraceback();
    }

    /**
     * Setup monk mode for LCS - user clicks cells
     */
    _setupMonkModeLCS() {
        const { s1, s2 } = this.algoData;
        let i = this.currentRow;
        let j = this.currentCol;
        let lcsChars = [];

        const processStep = (clickedRow, clickedCol) => {
            if (i <= 0 || j <= 0) {
                this.lcsResult = lcsChars.join('');
                this._completeTraceback();
                return;
            }

            const char1 = s1[i - 1];
            const char2 = s2[j - 1];

            let expectedRow, expectedCol;
            let isMatch = false;

            if (char1 === char2) {
                // Should go diagonal
                expectedRow = i - 1;
                expectedCol = j - 1;
                isMatch = true;
            } else {
                // Should go to max of top or left
                const topVal = this.dpTable[i - 1][j];
                const leftVal = this.dpTable[i][j - 1];

                if (topVal >= leftVal) {
                    expectedRow = i - 1;
                    expectedCol = j;
                } else {
                    expectedRow = i;
                    expectedCol = j - 1;
                }
            }

            if (clickedRow === expectedRow && clickedCol === expectedCol) {
                // Correct click
                this._markAsVisited(i, j);

                if (isMatch) {
                    lcsChars.unshift(char1);
                    this._addLogEntry({
                        step: `Match`,
                        position: `DP[${i}][${j}]`,
                        condition: `'${char1}' == '${char2}'`,
                        path: `✓ Match! Add '${char1}' to LCS`,
                        type: 'match'
                    });
                } else {
                    const topVal = this.dpTable[i - 1][j];
                    const leftVal = this.dpTable[i][j - 1];
                    const direction = topVal >= leftVal ? 'up' : 'left';

                    this._addLogEntry({
                        step: `No Match`,
                        position: `DP[${i}][${j}]`,
                        condition: `'${char1}' ≠ '${char2}'. Top=${topVal}, Left=${leftVal}`,
                        path: `→ Move ${direction}`,
                        type: `move-${direction}`
                    });
                }

                i = expectedRow;
                j = expectedCol;

                if (i <= 0 || j <= 0) {
                    this.lcsResult = lcsChars.join('');
                    this._completeTraceback();
                } else {
                    this._highlightCurrentCell(i, j);
                    this._enableCellClicks(processStep);
                }
            } else {
                // Wrong click
                this._showWrongClick(clickedRow, clickedCol);
            }
        };

        this._enableCellClicks(processStep);
    }

    /**
     * Enable cell clicks for monk mode
     */
    _enableCellClicks(callback) {
        // Remove previous handler if exists
        if (this.cellClickHandler) {
            this.visualizer.container.removeEventListener('click', this.cellClickHandler);
        }

        // Make all cells clickable
        this.visualizer.cells.flat().forEach(cell => {
            cell.classList.add('traceback-clickable');
        });

        this.cellClickHandler = (e) => {
            const cell = e.target.closest('.grid-cell');
            if (!cell || cell.classList.contains('header')) return;

            const row = parseInt(cell.dataset.row);
            const col = parseInt(cell.dataset.col);

            callback(row, col);
        };

        this.visualizer.container.addEventListener('click', this.cellClickHandler);
    }

    /**
     * Disable cell clicks
     */
    _disableCellClicks() {
        if (this.cellClickHandler) {
            this.visualizer.container.removeEventListener('click', this.cellClickHandler);
            this.cellClickHandler = null;
        }

        this.visualizer.cells.flat().forEach(cell => {
            cell.classList.remove('traceback-clickable');
        });
    }

    /**
     * Complete the traceback process
     */
    _completeTraceback() {
        this.isTracing = false;
        this.tracebackBtn.classList.remove('active');
        this.tracebackBtn.textContent = '🔍 Traceback Items';
        this._disableCellClicks();

        // Mark final path
        this._markTracePath();

        // Show result
        this.tracebackResult.classList.remove('hidden');

        if (this.algoType === 'knapsack') {
            const sortedItems = this.selectedItems.sort((a, b) => a - b);
            const itemsStr = sortedItems.length > 0
                ? sortedItems.map(i => `Item ${i}`).join(', ')
                : 'None';
            this.selectedItemsEl.textContent = itemsStr;

            this._addLogEntry({
                step: 'Complete',
                position: 'Traceback finished',
                condition: `Found ${sortedItems.length} item(s) in optimal solution`,
                path: `Selected: ${itemsStr}`,
                type: 'complete'
            });
        } else {
            const resultStr = this.lcsResult || 'Empty';
            this.selectedItemsEl.textContent = `"${this.lcsResult}" (Length: ${this.lcsResult.length})`;

            this._addLogEntry({
                step: 'Complete',
                position: 'Traceback finished',
                condition: `LCS length = ${this.lcsResult.length}`,
                path: `LCS: "${this.lcsResult}"`,
                type: 'complete'
            });
        }

        // Update status text
        document.getElementById('status-text').textContent =
            this.algoType === 'knapsack'
                ? `Traceback complete! Selected items: ${this.selectedItems.sort((a, b) => a - b).join(', ') || 'None'}`
                : `Traceback complete! LCS: "${this.lcsResult}"`;
    }

    /**
     * Stop/reset traceback
     */
    stopTraceback() {
        this.isTracing = false;
        this.tracebackBtn.classList.remove('active');
        this.tracebackBtn.textContent = '🔍 Traceback Items';
        this.tracebackCard.classList.add('hidden');
        this._clearTracebackHighlights();
        this._disableCellClicks();
    }

    // ==================== Helper Methods ====================

    _highlightCurrentCell(row, col) {
        // Clear previous current highlight
        this.visualizer.cells.flat().forEach(cell => {
            cell.classList.remove('traceback-current');
        });

        if (this.visualizer.cells[row] && this.visualizer.cells[row][col]) {
            this.visualizer.cells[row][col].classList.add('traceback-current');
        }
    }

    _markAsVisited(row, col) {
        if (this.visualizer.cells[row] && this.visualizer.cells[row][col]) {
            this.visualizer.cells[row][col].classList.remove('traceback-current');
            this.visualizer.cells[row][col].classList.add('traceback-visited');
        }
    }

    _markTracePath() {
        // Clear any remaining current highlight (purple) so green shows through
        this.visualizer.cells.flat().forEach(cell => {
            cell.classList.remove('traceback-current');
        });

        // Mark all visited cells as path
        this.visualizer.cells.flat().forEach(cell => {
            if (cell.classList.contains('traceback-visited')) {
                cell.classList.add('traceback-path');
            }
        });
    }

    _clearTracebackHighlights() {
        this.visualizer.cells.flat().forEach(cell => {
            cell.classList.remove(
                'traceback-current',
                'traceback-visited',
                'traceback-path',
                'traceback-clickable',
                'traceback-wrong'
            );
        });
    }

    _showWrongClick(row, col) {
        if (this.visualizer.cells[row] && this.visualizer.cells[row][col]) {
            const cell = this.visualizer.cells[row][col];
            cell.classList.add('traceback-wrong');
            setTimeout(() => {
                cell.classList.remove('traceback-wrong');
            }, 500);
        }
    }

    _addLogEntry({ step, position, condition, path, type }) {
        const entry = document.createElement('div');
        entry.className = `log-entry ${type}`;
        entry.innerHTML = `
            <div class="log-step">${step}</div>
            <div class="log-position">${position}</div>
            <div class="log-condition">⚡ ${condition}</div>
            <div class="log-path">→ ${path}</div>
        `;
        this.logContainer.appendChild(entry);

        // Auto scroll to bottom
        this.logContainer.scrollTop = this.logContainer.scrollHeight;
    }

    _addMonkHint(message) {
        const hint = document.createElement('div');
        hint.className = 'traceback-hint';
        hint.textContent = message;
        this.logContainer.insertBefore(hint, this.logContainer.firstChild);
    }

    _delay(ms) {
        return new Promise(resolve => setTimeout(resolve, ms));
    }

    setSpeed(ms) {
        this.animationSpeed = ms;
    }
}
